 ...
 * Desarrollado para: VARIEDADES CS
 * =========================================================================
 */
...
/**
 * PASO 5: Inicializa todas las hojas de cálculo con sus encabezados oficiales
 */
  ...
  const definiciones = [
    {
      ...
      encabezados: ['Código', 'Producto', 'Categoría', 'Existencia', 'Precio Compra', 'Precio Venta']
    },
    {
      nombre: HOJAS.MOVIMIENTOS,
      encabezados: ['Fecha', 'Código', 'Producto', 'Categoría', 'Tipo', 'Cantidad', 'Precio Unitario', 'Total']
    },
    {
      nombre: HOJAS.VENTAS,
      encabezados: ['N° Venta', 'Fecha', 'Código', 'Producto', 'Categoría', 'Cantidad', 'Precio Unitario', 'Total', 'Forma de Pago', 'Fecha Vencimiento', 'N° Crédito', 'Usuario', 'Estado', 'Fecha Anulación', 'Motivo']
    },
    {
      nombre: HOJAS.COMPRAS,
      encabezados: ['N° Compra', 'Fecha', 'Código', 'Producto', 'Categoría', 'Cantidad', 'Precio Unitario', 'Total']
    },
    {
      nombre: HOJAS.CLIENTES,
      encabezados: ['ID Cliente', 'Nombre', 'Teléfono', 'Dirección', 'Observaciones']
    },
    {
      nombre: HOJAS.PROVEEDORES,
      encabezados: ['ID Proveedor', 'Nombre', 'Teléfono', 'Dirección', 'Observaciones']
    },
    {
      nombre: HOJAS.CREDITOS,
      encabezados: ['N° Crédito', 'Fecha', 'ID Cliente', 'Cliente', 'N° Venta', 'Total Crédito', 'Abonado', 'Saldo', 'Vencimiento', 'Estado']
    },
    {
      nombre: HOJAS.ABONOS,
      encabezados: ['N° Abono', 'Fecha', 'N° Crédito', 'ID Cliente', 'Cliente', 'Monto Abonado', 'Método de Pago', 'Observaciones']
    },
    {
      nombre: HOJAS.CUENTAS_COBRAR,
      encabezados: ['ID Cliente', 'Cliente', 'Total Créditos', 'Total Abonado', 'Saldo Pendiente', 'Créditos Pendientes', 'Estado']
    },
    {
      nombre: HOJAS.CAJA,
      encabezados: ['ID', 'Fecha', 'Tipo', 'Concepto', 'Monto', 'Usuario', 'Saldo']
    },
    {
      nombre: HOJAS.INVENTARIO,
      encabezados: ['Código', 'Producto', 'Categoría', 'Existencia', 'Precio Compra', 'Precio Venta', 'Valor Total']
    },
    {
      nombre: HOJAS.DASHBOARD,
      encabezados: ['Métrica', 'Valor', 'Última Actualización']
    },
    {
      nombre: HOJAS.USUARIOS,
      encabezados: ['Usuario', 'Contraseña', 'Rol']
    }
  ];
...
/**
 * Autenticación de usuario validado desde Apps Script
 */
  ...
  try {
    ...
    if (encontrado) {
      return {
        ...
        mensaje: 'Bienvenido(a) a VARIEDADES CS'
      };
    } else {
...
/**
 * Carga inicial consolidada para el frontend
 */
...
function guardarProducto(producto) {
  try {
    ...
    if (!codigo || !nombre) {
      return { success: false, mensaje: 'Código y Nombre de producto son obligatorios.' };
    }
    ...
    if (filaExistente > 0) {
      sheet.getRange(filaExistente, 1, 1, 6).setValues([[
        codigo, nombre, categoria, existencia, precioCompra, precioVenta
      ]]);
    } else {
...
function eliminarProducto(codigo) {
  try {
    ...
    return { success: false, mensaje: 'Error al eliminar producto: ' + e.toString() };
  }
}

/**
 * REGISTRAR VENTA (Punto de Venta)
 * Valida stock, descuenta inventario, registra movimiento, actualiza caja o crédito.
 */
function registrarVenta(datos) {
  ...
  try {
    ...
    // 1. Validación estricta de inventario para todos los artículos
    for (let item of items) {
      ...
      if (!prod) {
        return { success: false, mensaje: 'El producto con código ' + cod + ' no existe en inventario.' };
      }
      if (prod.existencia < cant) {
        return {
          success: false,
          mensaje: 'Solo quedan ' + prod.existencia + ' unidades disponibles de "' + prod.nombre + '".'
        };
      }
    }
    ...
    if (formaPago === 'Crédito') {
      if (!datos.idCliente) {
        return { success: false, mensaje: 'Debe seleccionar un cliente para ventas a crédito.' };
      }
    ...
    // 3. Descontar inventario y registrar movimientos por producto
    items.forEach(item => {
      const cod = String(item.codigo).trim().toUpperCase();
      const cant = Number(item.cantidad) || 0;
      ...
      const nuevaExistencia = prod.existencia - cant;
      sheetProd.getRange(prod.fila, 4).setValue(nuevaExistencia);
    ...
    // 4. Actualizar Caja si el pago no fue a crédito (Efectivo, Tarjeta, Transferencia)
    if (formaPago !== 'Crédito') {
      const saldoActualCaja = obtenerSaldoCajaActual();
      const nuevoSaldo = saldoActualCaja + totalFinal;
    ...
    return { success: false, mensaje: 'Error al procesar venta: ' + e.toString() };
  } finally {
    lock.releaseLock();
  }
}
...
/**
 * ANULAR VENTA
 * No borra físicamente. Devuelve stock, registra movimiento inverso, reversa caja si aplica.
 */
function anularVenta(numeroVenta, motivo, usuario) {
  ...
  try {
    ...
    if (filasVenta.length === 0) {
      return { success: false, mensaje: 'No se encontró la venta ' + numClean + '.' };
    }

    // Validar si ya fue anulada
    if (filasVenta[0].estado === 'ANULADA') {
      return { success: false, mensaje: 'No se puede anular: La venta ' + numClean + ' ya está ANULADA.' };
    }
    ...
    const prodMap = {};
    for (let i = 0; i < prodData.length; i++) {
      prodMap[String(prodData[i][0]).trim().toUpperCase()] = {
        fila: i + 2,
        existencia: Number(prodData[i][3]) || 0
      };
    }
    ...
    // 1. Devolver cantidades al inventario y registrar movimientos inversos
    ...
    const formaPago = filasVenta[0].formaPago;
    const numCredito = filasVenta[0].numCredito;

    // 2. Revertir movimiento de Caja si no fue crédito
    if (formaPago !== 'Crédito') {
      const saldoActualCaja = obtenerSaldoCajaActual();
      const nuevoSaldo = saldoActualCaja - totalVentaRevertir;
      const idCaja = 'CJ-' + String(sheetCaja.getLastRow()).padStart(5, '0');
      ...
      sheetCaja.appendRow([
        ...
        fechaHora,
        'Egreso',
        'Reverso por Anulación de Venta ' + numClean + ' (Motivo: ' + motivo + ')',
        -totalVentaRevertir,
        userAnula,
        nuevoSaldo
      ]);
    } else if (numCredito && sheetCred) {
      // Si fue crédito, anular el crédito
      ...
      for (let i = 0; i < credData.length; i++) {
        if (String(credData[i][0]).trim().toUpperCase() === String(numCredito).trim().toUpperCase()) {
    ...
    return {
      success: true,
      mensaje: 'Venta ' + numClean + ' ha sido ANULADA con éxito. Inventario y finanzas restituidos.'
    };
  } catch (e) {
    return { success: false, mensaje: 'Error al anular venta: ' + e.toString() };
  } finally {
    lock.releaseLock();
  }
}
...
/**
 * Obtener listado de ventas agrupadas
 */
...
/**
 * REGISTRAR COMPRA (Aumenta inventario y registra movimiento)
 */
function registrarCompra(datos) {
  ...
  try {
    ...
    const items = datos.items || [];
    if (!items.length) {
      return { success: false, mensaje: 'Debe agregar al menos un producto a la compra.' };
    }
    ...
    items.forEach(item => {
      ...
      if (prodMap[cod]) {
        ...
        sheetProd.getRange(fila, 5).setValue(pCompra); // Actualiza último precio compra
    ...
    // Registrar egreso de caja si se pagó desde caja
    if (datos.pagarDesdeCaja) {
      const saldoActualCaja = obtenerSaldoCajaActual();
      const nuevoSaldo = saldoActualCaja - totalCompra;
      const idCaja = 'CJ-' + String(sheetCaja.getLastRow()).padStart(5, '0');
      sheetCaja.appendRow([
        idCaja,
        fechaHora,
        'Egreso',
        'Pago Compra Mercadería ' + numCompra + ' (' + (datos.proveedor || '') + ')',
        -totalCompra,
        usuario,
        nuevoSaldo
      ]);
    ...
    return {
      success: true,
      mensaje: 'Compra ' + numCompra + ' registrada e inventario actualizado con éxito.',
...
/**
 * CLIENTES: Agregar, editar, obtener
 */
  ...
  try {
    ...
    if (fila > 0) {
      sheet.getRange(fila, 2, 1, 4).setValues([[
        nombre, cliente.telefono || '', cliente.direccion || '', cliente.observaciones || ''
      ]]);
    } else {
    ...
    return { success: false, mensaje: 'Error al guardar cliente: ' + e.toString() };
  }
}

/**
 * PROVEEDORES: Agregar, editar, obtener
 */
...
function guardarProveedor(proveedor) {
  try {
    ...
    if (fila > 0) {
      sheet.getRange(fila, 2, 1, 4).setValues([[
        nombre, proveedor.telefono || '', proveedor.direccion || '', proveedor.observaciones || ''
      ]]);
    } else {
    ...
    return { success: false, mensaje: 'Error al guardar proveedor: ' + e.toString() };
  }
}
...
function obtenerCreditos() {
  ...
  return data.map((r, i) => {
    ...
    if (estado !== 'ANULADO') {
      if (saldo <= 0.01) {
        estado = 'PAGADO';
      } else if (!isNaN(vtoDate.getTime()) && vtoDate < hoy) {
        estado = 'VENCIDO';
      } else {
        estado = 'PENDIENTE';
      }
 ...
 * Valida: "No permitir abonar una cantidad mayor al saldo"
 */
function registrarAbono(datos) {
  ...
  try {
    ...
    if (monto <= 0) return { success: false, mensaje: 'El monto a abonar debe ser mayor a 0.' };
    ...
    if (monto > creditoActual.saldo + 0.001) {
      return {
        success: false,
        mensaje: 'No se puede abonar ' + monto.toFixed(2) + '. El saldo pendiente es de ' + creditoActual.saldo.toFixed(2) + '.'
      };
    }
    ...
    // 1. Guardar en Abonos
    sheetAbonos.appendRow([
      numAbono,
      fechaHora,
      numCred,
      creditoActual.idCliente,
      creditoActual.cliente,
      monto,
      metodo,
      datos.observaciones || 'Abono a crédito'
    ]);

    // 2. Actualizar Crédito
    sheetCred.getRange(filaCred, 7, 1, 4).setValues([[
      nuevoAbonado,
      nuevoSaldo,
      credData[filaCred - 2][8], // Conservar vencimiento
      nuevoEstado
    ]]);
    ...
    sheetCaja.appendRow([
      ...
      fechaHora,
      'Abono',
      'Abono ' + numAbono + ' a Crédito ' + numCred + ' (' + creditoActual.cliente + ')',
      monto,
      usuario,
    ...
    return {
      success: true,
      mensaje: 'Abono ' + numAbono + ' registrado por $' + monto.toFixed(2) + '. Nuevo saldo: $' + nuevoSaldo.toFixed(2),
...
/**
 * CUENTAS POR COBRAR (Sincronización agrupada por cliente)
 */
function recalcularCuentasPorCobrar() {
  ...
  credData.forEach(r => {
    ...
    if (saldo > 0.01) {
      clientesMap[idCli].creditosPendientes += 1;
    }
  });

  // Limpiar hoja de Cuentas por Cobrar manteniendo encabezado
  ...
  const filas = Object.values(clientesMap).map(c => [
    ...
    c.totalAbonado,
    c.saldoPendiente,
    c.creditosPendientes,
    c.saldoPendiente > 0.01 ? 'PENDIENTE' : 'AL DÍA'
  ]);

  if (filas.length > 0) {
    sheetCxc.getRange(2, 1, filas.length, 7).setValues(filas);
  }
}
...
function obtenerCaja() {
  ...
  const movimientos = data.map(r => {
    ...
    if (tipo.toLowerCase().includes('inicial')) {
      saldoInicial += monto;
    } else if (monto > 0) {
      ingresos += monto;
    } else {
...
function registrarMovimientoCaja(datos) {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const sheet = ss.getSheetByName(HOJAS.CAJA);
    const tipo = datos.tipo || 'Ingreso'; // Ingreso, Egreso, Ajuste
    ...
    sheet.appendRow([
      ...
      fechaHora,
      tipo,
      datos.concepto || ('Movimiento manual de ' + tipo),
      monto,
      datos.usuario || 'SISTEMA',
      nuevoSaldo
    ]);

    return { success: true, mensaje: 'Movimiento de caja registrado exitosamente.', nuevoSaldo: nuevoSaldo };
  } catch (e) {
    return { success: false, mensaje: 'Error al registrar movimiento de caja: ' + e.toString() };
  }
}
...
function cerrarCaja(usuario, observaciones) {
  try {
    ...
    sheet.appendRow([
      ...
      fechaHora,
      'Cierre de Caja',
      'Cierre de Turno. ' + (observaciones || 'Sin observaciones.'),
      0,
      usuario || 'SISTEMA',
      saldoActual
    ]);
    ...
    return {
      success: true,
      mensaje: 'Cierre de caja completado. Saldo final: $' + saldoActual.toFixed(2),
      saldoFinal: saldoActual
    };
  } catch (e) {
    return { success: false, mensaje: 'Error al cerrar caja: ' + e.toString() };
  }
}
...
function obtenerSaldoCajaActual() {
  ...
  const ultimoSaldo = sheet.getRange(sheet.getLastRow(), 7).getValue();
  return Number(ultimoSaldo) || 0;
}
...
function actualizarHojaInventario() {
  try {
    ...
    if (sheetInv.getLastRow() > 1) {
      sheetInv.getRange(2, 1, sheetInv.getLastRow() - 1, 7).clearContent();
    }
    ...
    const rows = prods.map(p => {
      const cant = Number(p[3]) || 0;
      const pCompra = Number(p[4]) || 0;
      return [
        p[0], // Código
        p[1], // Producto
        p[2], // Categoría
        cant, // Existencia
        pCompra, // Precio Compra
        p[5], // Precio Venta
        cant * pCompra // Valor Total
      ];
    });

    if (rows.length > 0) {
      sheetInv.getRange(2, 1, rows.length, 7).setValues(rows);
    }
  } catch (e) {
...
function obtenerDashboard() {
  ...
  // Mapa de precio de compra para calcular ganancia real
  const prodCostMap = {};
  prods.forEach(p => { prodCostMap[p.codigo] = p.precioCompra; });

  ventas.forEach(v => {
    if (v.estado !== 'ANULADA') {
      const vFechaStr = String(v.fecha);
      if (vFechaStr.includes(hoyStr)) {
        ventasDelDia += v.total;
      }
  ...
  const cantidadProductos = prods.length;
  const existenciaTotal = prods.reduce((acc, p) => acc + p.existencia, 0);
  const valorInventario = prods.reduce((acc, p) => acc + (p.existencia * p.precioCompra), 0);
